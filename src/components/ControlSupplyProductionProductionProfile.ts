...
export default class ControlSupplyProductionProductionProfile extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "code";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/supply/production/productionProfile${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }
