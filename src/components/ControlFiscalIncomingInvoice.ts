...
export default class ControlFiscalIncomingInvoice extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "id";
  }

  protected getAsyncIteratorZen(
    ...
    return new AsyncIteratorZen(
      `/fiscal/incomingInvoice${params.toString() ? `?${params.toString()}` : ""
      }`,
    );
  }
  ...
  protected getString(value: any): string | null {
    ...
    result.push(`${(<any> window).zen?.ctx.i18n.formatDate(value.date, value.company.locale)}`);
