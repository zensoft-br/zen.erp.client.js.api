...
export default class ControlCatalogCategory extends ControlSelectZen<any> {
  constructor() {
    super();
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/catalog/category${params.toString() ? `?${params.toString()}` : ""
      }`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.code === value.description ? value.code : `${value.code}, ${value.description}`}` : null;
  }
}
