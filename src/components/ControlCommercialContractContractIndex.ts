...
export default class ControlCommercialContractContractIndex extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "code";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/commercial/contract/contractIndex${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }
