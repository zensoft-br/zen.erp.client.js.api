...
export default class ControlFinancialReceivable extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "code";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/financial/receivable${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.status}, ${value.type}, ${value.code}, ${value.person.fantasyName ?? value.person.name}, $${value.value}` : null;
  }
}
