...
export default class ControlSystemPrintingPrinter extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "code";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/system/printing/printer${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.id}, ${value.description ?? value.code}` : null;
  }
}
