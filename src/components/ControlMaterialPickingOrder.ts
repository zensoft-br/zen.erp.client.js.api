...
export default class ControlMaterialPickingOrder extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "id";
  }
  ...
  protected getString(value: any): string | null {
    return value ? `${value.id}, ${value.status}${value.description ? `, ${value.description}` : null}` : null;
  }
}
