...
export default class ControlSystemWorkflowWorkflowNode extends ControlSelectZen<any> {
  constructor() {
    super();
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    // q
    {
      ...
      q.push("workflow.status==APPROVED");
    ...
    return new AsyncIteratorZen(
      `/system/workflow/workflowNode${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.workflow.description}, ${value.description}` : null;
  }
}
