...
export default class ControlMaterialAddress extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "code";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/material/address${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.warehouse.code}, ${value.code}` : null;
  }
}
