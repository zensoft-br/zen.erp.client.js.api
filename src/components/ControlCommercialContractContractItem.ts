...
export default class ControlCommercialContractContractItem extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "contract.code,description";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    // q
    {
      ...
      if (args?.text) {
        ...
        tmp.push(`contract.code=ilike=${`'${args.text}%'`}`);
    ...
    return new AsyncIteratorZen(
      `/commercial/contract/contractItem${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.contract.code}, ${value.description}` : null;
  }
}
