...
export default class ControlSystemMailMailerConfig extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "code";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/system/mail/mailerConfig${params.toString() ? `?${params.toString()}` : ""
      }`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.code}, ${(<any>window).zen?.ctx.i18n.getResource(value.description)}` : null;
  }
}
