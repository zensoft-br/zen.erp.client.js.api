...
export default class ControlFiscalFiscalProfilePerson extends ControlSelectZen<any> {
  constructor() {
    super();
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    // order
    params.append("order", args?.order || "description");

    return new AsyncIteratorZen(
      `/fiscal/fiscalProfilePerson${params.toString() ? `?${params.toString()}` : ""
      }`,
    );
  }
