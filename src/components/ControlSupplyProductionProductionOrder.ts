...
export default class ControlSupplyProductionProductionOrder extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "id";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/supply/production/productionOrder${params.toString() ? `?${params.toString()}` : ""
      }`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.id}, ${value.status}, ${value.date}, ${
      value.company.code}, ${value.person.fantasyName ?? value.person.name}` : null;
  }
}
