...
export default class ControlMaterialReservationTarget extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "id";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/material/reservationTarget${params.toString() ? `?${params.toString()}` : ""
      }`,
    );
  }
  ...
  protected getString(value: any): string | null {
    return value
      ? `${value.productPacking.product.description} ${value.productPacking.grid?.code ?? ""
      } ${value.productPacking.units ?? ""}`
      : null;
  }
}
