...
export default class ControlMaterialOutgoingList extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "id";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/material/outgoingList${params.toString() ? `?${params.toString()}` : ""
      }`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.id}, ${value.status}, ${value.date}, ${value.person.name}` : null;
  }
}
