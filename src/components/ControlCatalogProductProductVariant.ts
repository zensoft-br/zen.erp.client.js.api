...
export default class ControlCatalogProductProductVariant extends ControlSelectZen<any> {
  constructor() {
    super();
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/catalog/product/productVariant${params.toString() ? `?${params.toString()}` : ""
      }`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${value.code ? `${value.code}, ` : ""}${value.description}` : null;
  }
}
