...
export default class ControlSystemSecurityUser extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "name";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/system/security/user${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }

  protected getString(value: any): string | null {
    return value ? `${(<any> window).zen?.ctx.i18n.getResource(value.name)}` : null;
  }
}
