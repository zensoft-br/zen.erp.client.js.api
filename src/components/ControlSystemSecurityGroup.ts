...
export default class ControlSystemSecurityGroup extends ControlSelectZen<any> {
  constructor() {
    super();
    this._order = "code";
  }
  ...
  protected getAsyncIteratorZen(args: ControlSelectZenArgs): AsyncIterator<any> {
    ...
    return new AsyncIteratorZen(
      `/system/security/group${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }

  protected getString(value: any): string | null {
    return value
      ? `${value.code}, ${(<any> window).zen?.ctx.i18n.getResource(value.description)}`
      : null;
  }
}
